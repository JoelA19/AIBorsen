    ...
    if message.content.startswith('$test'):
        for stock in stocks:
            if stocks[stock][-1] > 70:
                embed = discord.Embed(
                    title=stock,
                    description=('Sell ' + stock + ', RSI: ' + str(int(stocks[stock][-1]))),
                    colour=discord.Colour.purple()
                )
                await message.channel.send(embed=embed, file=discord.File("pictures/" + stock + ".png"))

            elif stocks[stock][-1] < 30:
                embed = discord.Embed(
                    title=stock,
                    description=('Buy ' + stock + ', RSI: ' + str(int(stocks[stock][-1]))),
                    colour=discord.Colour.purple()
                )
                await message.channel.send(embed=embed, file=discord.File("pictures/" + stock + ".png"))
